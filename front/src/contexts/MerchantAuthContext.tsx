...
interface MerchantAuthContextType {
  ...
  merchant_register: (name: string, password: string, phone: string) => Promise<void>;
...
export const MerchantAuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const merchant_login = async (account: string, password: string) => {
    try {
      const response = await merchantLogin({ strUserName: account, strPassword: password });
  ...
  const merchant_register = async (name: string, password: string, phone: string) => {
    try {
      const response = await merchantRegister({
        strMerchantName: name,
      ...
      // 注册成功后不需要设置商家状态，直接导航到登录页面
      // 根据 API 返回值示例，只返回 merchant_id，不包含完整商家信息和 token
      setTimeout(() => {
        navigate('/login?tab=merchant', { replace: true });
      }, 100);
    } catch (error) {
