...
const MerchantCard: React.FC<{ merchant: Merchant }> = ({ merchant }) => {
  ...
  const merchantId = merchant?.iMerchantId?.toString() || '';
  ...
  return (
    <Link to={`/merchants/${merchant.iMerchantId}`}>
      ...
      >
        <CardCover>
          <Base64Image
            imageUrl={merchant.strCoverImage_url || merchant.strLogo_url}
            alt={merchant.strMerchantName}
        ...
        <CardContent>
          <Space direction="vertical" size={4} style={{ width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <Title level={5} style={{ margin: 0 }}>{merchant.strMerchantName || '商家名称未知'}</Title>
              <Rate disabled defaultValue={merchant.dRating || 0} style={{ fontSize: 12 }} />
            </div>

            <TagsWrapper>
              <Tag color="green">{merchant.strMerchantCategory || '未分类'}</Tag>
            </TagsWrapper>
            ...
            <InfoGrid>
              <InfoItem>
                ...
                <span className="info-value">{(merchant.tDeliveryTime || 30)}-{(merchant.tDeliveryTime || 30) + 10}分钟</span>
              </InfoItem>

              <InfoItem>
                <DollarSign size={16} />
                <span className="info-label">人均消费</span>
                <span className="info-value">¥{merchant.dAveragePrice || 0}/人</span>
              </InfoItem>

              <InfoItem>
                <DollarSign size={16} />
                <span className="info-label">起送价</span>
                <span className="info-value">¥{merchant.dMinOder || 0}</span>
              </InfoItem>
            </InfoGrid>
            ...
            <FooterInfo>
              {(merchant.dDeliveryFee ?? 0) === 0 ? (
                ...
                <Text type="secondary">配送费: ¥{(merchant.dDeliveryFee ?? 0).toFixed(2)}</Text>
              )}

              <Space size={8}>
                {(merchant.dRating ?? 0) >= 4.5 && (
